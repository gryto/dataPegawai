...
class LoginCopyPage extends StatefulWidget {
  final String tkn;
  const LoginCopyPage({Key? key, required this.tkn}) : super(key: key);

  @override
  State<LoginCopyPage> createState() => _LoginCopyPageState();
}

class _LoginCopyPageState extends State<LoginCopyPage> {
  String _token = '';
  String? token;
  ...
  //   return directory.path + '/shared_token.txt';
  // }

  // Future<String> _getSharedFilePathId() async {
  //   Directory directory = await getApplicationDocumentsDirectory();
  //   return directory.path + '/shared_id.txt';
  // }

  Future<void> _saveTokenToStorage(String token, String id) async {
    await storage.write(key: "token", value: token);
    await storage.write(key: "id", value: id);
    print('Token and ID saved securely');
    shareToken(token, id);
  }

  void shareToken(String token, String id) {
    // Replace with your second app's custom URL scheme
    final String deepLink = 'myapp://auth?token=$token&id=$id';
    Clipboard.setData(ClipboardData(text: deepLink));
    print('Deep link copied to clipboard: $deepLink');
    // Optionally launch the second app
    // _launchURL(deepLink);
  }
  ...
  //       String savePath = '$directoryPath/shared_token.txt';
  //       String savePathId = '$directoryPath/shared_id.txt';

  //       await File(savePath).writeAsString(token);
  //       await File(savePathId).writeAsString(id);
  ...
  //     } else {
  //       print('Token or ID not found in secure storage');
  //       _onAlertButtonPressed(context, false, "Token or ID not found.");
  //     }
  //   } catch (e) {
  //     print('Error reading from secure storage: $e');
  //     _onAlertButtonPressed(context, false, "Failed to download token file.");
  //   }
  // }
  ...
  loginCheck(username, password, tkn) async {
    ...
    try {
      ...
      if (content['status'] == 200) {
        ...
        // Save token and ID to secure storage
        await _saveTokenToStorage(accessToken, userId);
